#!/usr/bin/env bash
# ==============================================================================
#  AXIOM UPDATER (v8.0) — ARCHITECTURE & USAGE MANUAL
# ==============================================================================
...
# CONFIGURATION
# ==============================================================================
declare -r DOTFILES_GIT_DIR="${HOME}/axiom"
...
declare -r BACKUP_BASE_DIR="${HOME}/Documents/axiom_backups"
declare -r STATE_HOME_DIR="${XDG_STATE_HOME:-${HOME}/.local/state}/axiom"
declare -r FALLBACK_LOG_BASE_DIR="${STATE_HOME_DIR}/logs"
declare -r FALLBACK_BACKUP_BASE_DIR="${STATE_HOME_DIR}/backups"
declare -r REPO_URL="https://github.com/dusklinux/axiom"
declare -r BRANCH="main"
declare -r UPSTREAM_REMOTE="axiom-upstream"
declare -r UPSTREAM_TRACKING_REF="refs/axiom-updater/upstream/${BRANCH}"
...
declare -A CUSTOM_SCRIPT_PATHS=(
    ...
    ["update_checker.sh"]="user_scripts/update_axiom/update_checker/update_checker.sh"
    ["cc_restart.sh"]="user_scripts/axiom_system/reload_cc/cc_restart.sh"
    ["axiom_service_manager.sh"]="user_scripts/services/axiom_service_manager.sh"
    ...
    ["axiom_commands_before.sh"]="user_scripts/misc_extra/axiom_commands_before.sh"
    ["axiom_commands_after.sh"]="user_scripts/misc_extra/axiom_commands_after.sh"
    ["rofi_wallpaper_selctor.sh"]="user_scripts/rofi/rofi_wallpaper_selctor.sh"
    ["hypr_anim.sh"]="user_scripts/rofi/hypr_anim.sh"
    ["axiom_matugen_config_tui.sh"]="user_scripts/theme_matugen/axiom_matugen_config_tui.sh"
    ["axiom_firefox_tui.sh"]="user_scripts/theme_matugen/axiom_firefox_tui.sh"
...
desktop_notify() {
    ...
    local summary="${2:-Axiom Update}"
    local body="${3:-}"

    if command -v notify-send &>/dev/null; then
        timeout 3 notify-send --urgency="$urgency" --app-name="Axiom Updater" "$summary" "$body" \
            >/dev/null 2>&1 || true
    fi
}
...
show_help() {
    cat <<'HELPEOF'
Axiom Updater — Dotfile sync and setup tool for Arch Linux / Hyprland

Usage: axiom_updater.sh [OPTIONS]
...
Logs are saved to:
  ~/Documents/logs/
  Fallback: ~/.local/state/axiom/logs/

Backups are saved to:
  ~/Documents/axiom_backups/
  Fallback: ~/.local/state/axiom/backups/
HELPEOF
}

show_version() {
    printf 'Axiom Updater v%s\n' "$VERSION"
}
...
setup_runtime_dir() {
    ...
    if [[ -n "${XDG_RUNTIME_DIR:-}" ]]; then
        candidate="${XDG_RUNTIME_DIR}/axiom-updater"
    ...
    candidate="/tmp/axiom-updater-${EUID}"
    if ! ensure_secure_runtime_dir "$candidate"; then
...
setup_logging() {
    LOG_FILE="$(make_private_file_under "$ACTIVE_LOG_BASE_DIR" "axiom_update_${RUN_TIMESTAMP}_XXXXXX.log")" || {
...
auto_prune() {
    if [[ -d "$ACTIVE_LOG_BASE_DIR" ]]; then
        find "$ACTIVE_LOG_BASE_DIR" -type f -name 'axiom_update_*.log' -mtime "+${LOG_RETENTION_DAYS}" -delete \
            2>/dev/null || true
    fi
...
backup_worktree_collisions_for_ref() {
    ...
    {
        printf 'Axiom work-tree collision backup\n'
        printf 'Created: %s\n' "$RUN_TIMESTAMP"
        printf 'Reference: %s\n' "$ref"
        printf 'Work tree: %s\n' "$WORK_TREE"
    } > "$info_file" || {
...
handle_unrelated_upstream_history() {
    ...
    if [[ -t 0 ]]; then
        printf '\n%s[UNRELATED HISTORY]%s The existing bare repo at %s is not based on Axiom upstream.\n' \
...
backup_full_tracked_tree() {
    ...
    printf 'Axiom full tracked-tree backup\n' >> "$info_file"
...
backup_git_history() {
    ...
    {
        printf 'Axiom Git history backup\n'
        printf 'Created: %s\n' "$RUN_TIMESTAMP"
        printf 'Source: %s\n' "$DOTFILES_GIT_DIR"
    } > "$info_file" || true
    chmod 600 -- "$info_file" 2>/dev/null || true
...
atomic_restore_path() {
    ...
    tmpdir="$(mktemp -d -p "$parent" ".${base}.axiom_tmp.XXXXXX")" || return 1
    ...
    for path in "${CHANGE_PATHS[@]}"; do
        ...
        case "$action" in
            ...
            delete-merge)
                ...
                marker="${MERGE_DIR}/${path}.axiom_deleted"
                ensure_relative_parent_dir "$MERGE_DIR" "${path}.axiom_deleted" mkdir_cache || {
...
cleanup() {
    ...
    if ((${#HARD_FAILED_SCRIPTS[@]} > 0)); then
        desktop_notify critical "Axiom Update" "${#HARD_FAILED_SCRIPTS[@]} required script(s) failed"
        exit 1
    elif ((rc != 0)); then
        desktop_notify critical "Axiom Update" "Update failed or interrupted"
        exit "$rc"
    elif [[ "$SYNC_FAILED" == true ]]; then
        desktop_notify critical "Axiom Update" "Sync phase failed"
        exit 1
    else
        desktop_notify normal "Axiom Update" "Update completed successfully"
        exit 0
    fi
}
