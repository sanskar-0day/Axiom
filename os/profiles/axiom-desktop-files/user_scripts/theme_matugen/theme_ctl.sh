...
#
# Architecture:
#   1. INTERNAL STATE: ~/.config/axiom/settings/axiom_theme/state.conf
#   2. PUBLIC STATE:   ~/.config/axiom/settings/axiom_theme/state (true/false)
...
# --- CONFIGURATION ---
readonly STATE_DIR="${HOME}/.config/axiom/settings/axiom_theme"
...
write_state() {
    ...
    if (( ! had_content )); then
        printf '%s\n' "# Axiom Theme State File" > "$_TEMP_FILE"
    fi
