...
# ==============================================================================
# AXIOM KOKORO INSTALLER V36 (AMD Fix + Model Precision + Final Polish)
# ==============================================================================

readonly SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
readonly ENV_DIR="$HOME/contained_apps/uv/axiom_kokoro"
readonly MODEL_DIR="$ENV_DIR/models"
readonly TRIGGER_DIR="$HOME/user_scripts/tts_stt/axiom_kokoro"
...
echo ":: [V36] Initializing Axiom Kokoro Setup..."
...
if [[ -f "$SCRIPT_DIR/axiom_main.py" ]]; then
    cp "$SCRIPT_DIR/axiom_main.py" "$ENV_DIR/"
    echo ":: axiom_main.py deployed."
else
    echo ":: ERROR: axiom_main.py not found in current directory."
    exit 1
fi
...
#!/usr/bin/env bash
# Axiom Kokoro Trigger V36 ($MODE edition)
# Features: Universal HW, Robust Detection, Cold Boot Fix, Hard Kill, Base64 IPC

readonly APP_DIR="$HOME/contained_apps/uv/axiom_kokoro"
readonly PID_FILE="/tmp/axiom_kokoro.pid"
readonly READY_FILE="/tmp/axiom_kokoro.ready"
readonly FIFO_PATH="/tmp/axiom_kokoro.fifo"
readonly DAEMON_LOG="/tmp/axiom_kokoro.log"
readonly DEBUG_LOG="\$APP_DIR/axiom_debug.log"
...
start_daemon() {
    ...
    if [[ "\$debug_mode" == "true" ]]; then
        ...
        export AXIOM_LOG_FILE="\$DEBUG_LOG"
        nohup uv run axiom_main.py --daemon --debug-file "\$DEBUG_LOG" > "\$DAEMON_LOG" 2>&1 &
    else
        nohup uv run axiom_main.py --daemon > "\$DAEMON_LOG" 2>&1 &
    fi
...
show_help() {
    cat << 'HELP'
Axiom Kokoro TTS — Trigger Script
