...
# Target: Arch Linux / Hyprland / Axiom / UWSM
# -----------------------------------------------------------------------------
...
readonly SETTINGS_DIR="$HOME/.config/axiom/settings"
readonly FAVORITES_FILE="$SETTINGS_DIR/axiom_theme/wal_fav_rofi"
readonly STATE_FILE="$SETTINGS_DIR/axiom_theme/state.conf"
readonly FAV_STATE_FILE="$SETTINGS_DIR/axiom_theme/current_fav"
...
update_tracker() {
  ...
  local mode
  mode=$(get_theme_mode)
  local track_file="${SETTINGS_DIR}/axiom_theme/${mode}_wal"
  
  # Atomic write ported from theme_ctl.sh
  local tmp_track
  tmp_track=$(mktemp "${SETTINGS_DIR}/axiom_theme/track.tmp.XXXXXX")
...
update_fav_state() {
  ...
  # Atomic write ported from theme_ctl.sh
  local tmp_state
  tmp_state=$(mktemp "${SETTINGS_DIR}/axiom_theme/current_fav.tmp.XXXXXX")
