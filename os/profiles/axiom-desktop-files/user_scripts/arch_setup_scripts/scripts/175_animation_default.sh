#!/usr/bin/env bash
# set Hyprland animation config to axiom (Default)
# -----------------------------------------------------------------------------
# Purpose: Copy 'axiom.conf' to 'active.conf' & reload Hyprland
...
# --- Configuration ---
readonly SOURCE_FILE="${HOME}/.config/hypr/source/animations/horizontal_axiom.conf"
...
main() {
    ...
    if cp -- "$SOURCE_FILE" "$TARGET_FILE"; then
        ...
        printf "[${C_GREY}%s${C_RESET}] ${C_BLUE}[INFO]${C_RESET}  Switched animation to: ${C_GREEN}axiom${C_RESET}\n" \
            "$(date +%T)"
    else
