...
# ==============================================================================
#  AXIOM FLEET PATCHER (Enterprise Edition - V3)
...
declare -ra FLEET_COMMANDS=(
    ...
    'U | "$HOME/user_scripts/axiom_system/reload_cc/cc_restart.sh" --quiet &'
    'U | "$HOME/user_scripts/sliders/reload_sliders/reload_sliders.sh" --quiet &'
    'U | mkdir -p "$HOME/Documents/axiom_backups/"'

    # --- System Services ---
#    "U | systemctl --user disable axiom.service || true"
...
# 2. Paths & Constants
readonly STATE_DIR="${HOME}/.local/state/axiom"
readonly STATE_FILE="${STATE_DIR}/patch_history.state"
readonly LOG_FILE="${HOME}/Documents/logs/axiom_patcher_$(date +%Y%m%d_%H%M%S).log"
readonly LOCK_FILE="${XDG_RUNTIME_DIR:-/run/user/$UID}/axiom_fleet_patcher.lock"
